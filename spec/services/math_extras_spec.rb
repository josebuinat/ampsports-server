...
describe MathExtras do
  describe '#substract_ranges' do
    it 'should return empty array if substracted range covers given' do
      expect(MathExtras.subtract_ranges([(1..10)], [(1..10)])).to eq []
    end

    it 'should return given array if substracted range not covers given' do
      expect(MathExtras.subtract_ranges([(1..10)], [(11..20)])).to eq [(1..10)]
    end

    it 'should return array with lesser range if substracted range overlaps start of given' do
      expect(MathExtras.subtract_ranges([(1..10)], [(0..3)])).to eq [(4..10)]
    end

    it 'should return array with lesser range if substracted range overlaps end of given' do
      expect(MathExtras.subtract_ranges([(1..10)], [(7..12)])).to eq [(1..6)]
    end

    it 'should return array of lesser ranges if substracted range overlaps middle of given' do
      expect(MathExtras.subtract_ranges([(1..10)], [(5..7)])).to eq [(1..4), (8..10)]
    end

    it 'should treat same numbers as intersection' do
      expect(MathExtras.subtract_ranges([(1..1)], [(1..2)])).to eq []
      expect(MathExtras.subtract_ranges([(2..2)], [(1..2)])).to eq []
      expect(MathExtras.subtract_ranges([(2..3)], [(1..2), (3..4)])).to eq []
      expect(MathExtras.subtract_ranges([(1..2), (2..3)], [(2..2)])).to eq [(1..1), (3..3)]
    end

    it 'should return array of lesser ranges if substracted ranges overlaps parts of given' do
      expect(MathExtras.subtract_ranges([(1..10)], [(3..4), (7..12)])).to eq [(1..2), (5..6)]
      expect(MathExtras.subtract_ranges([(7..25)], [(3..4), (8..12), (7..13), (20..24)])).to eq [(14..19), (25..25)]
      expect(MathExtras.subtract_ranges([(6..25)], [(3..4), (8..12), (7..13), (20..24)])).to eq [(6..6), (14..19), (25..25)]
    end
  end
  ...
  describe '#round_to_half_decimal' do
    let(:test_data) {{ -1.4 => -1.5, -1.1 => -1, 0 => 0, 3.222 => 3 }}

    it 'should round to nearest .5' do
      test_data.each do |input, output|
        expect(MathExtras.round_to_half_decimal(input)).to eq(output)
      end
    end
