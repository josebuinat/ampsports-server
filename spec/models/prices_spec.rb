...
describe "prices" do
  ...
  describe "#minimum_price" do
    ...
    let(:price) { build :filled_price, price: price_value, courts: [court] }
    let(:price_value) { 0 }
    ...
    context 'with Finnish venue' do
      ...
      context 'when price value is negative' do
        let(:price_value) { -1 }

        it 'adds a price error' do
          expect(price.errors.messages).to include(:price)
        end
      end

      context 'when price value is zero' do
        it 'does not add any errors' do
          expect(price.errors).to be_none
        end
      end

      context 'when price value is 2' do
        let(:price_value) { 2 }
    ...
    context 'with US venue' do
      ...
      context 'when price value is zero' do
        it 'adds a minimum price error' do
          expect(price.errors.messages).to include(price: [I18n.t('errors.price.minimum')])
        end
      end
