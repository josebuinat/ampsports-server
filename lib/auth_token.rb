class AuthToken
  ...
  def self.encode(payload, ttl_in_minutes: DEFAULT_TTL, key_field: :secret_key_base)
    payload[:exp] = ttl_in_minutes.minutes.from_now.to_i
    JWT.encode(payload, Rails.application.secrets.public_send(key_field))
  end
  ...
  def self.decode(token, leeway: nil, key_field: :secret_key_base)
    decoded = JWT.decode(token, Rails.application.secrets.public_send(key_field), leeway: leeway)
