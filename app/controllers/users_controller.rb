class UsersController < ApplicationController
  def show
    ...
    when 'future'
      @user.future_reservations.include_venues.order(start_time: :asc)
    when 'past'
      @user.past_reservations.include_venues.order(start_time: :desc)
    else
      @mode = 'future'
      @user.future_reservations.include_venues.order(start_time: :asc)
    end
  ...
  def invoices
    @user = current_user
    @reservations = current_user.reservations.future.order(start_time: :asc)
