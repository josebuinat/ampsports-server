...
module TimeSanitizer
  ...
  # then changed IMMEDIATELY to UTC
  def self.input(date_time)
    # better nil than 500 error?
    return nil if date_time.blank?
    # say, every string with a colon has time inside
    has_time = date_time.respond_to?(:include?) && date_time.include?(':')
    if has_time
      Time.zone.parse(date_time).utc
    else
  ...
  def self.duration(from, to)
    ((input(to.to_s) - input(from.to_s))).to_i
  end
end
