$(function() {
  var selectAll = function(elements, e) {
    elements.prop('checked', $(e.target).prop('checked'));
    elements.trigger('change');
  }
  $(document).on('click', '.js-outstanding-select-all', function(e) {
    selectAll($('input[name="user_ids[]"'), e);
  });
  $(document).on('click', '.js-invoice-select-all', function(e) {
    selectAll($('input[name="invoice_ids"'), e);
  });
