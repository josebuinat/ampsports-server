$(function () {
  ...
  $(document).on('change', '#court-private', function(e) {
...
function loadCourtEditSucc(resp) {
  ...
  $('#new_court input[name=copies]').val(1);
...
function courts_indexes_select_update() {
  ...
  var custom_name = $('#court_custom_name').val();

  if ((sport_name && indoor) || custom_name) {
    ...
    $.ajax({
      ...
      data: {
        ...
        existing_court: $('#court-indexes-select').data('id'),
        copies: copies },
...
function courts_indexes_select_add_form_listeners() {
  ...
  $('#new_court input[name=copies], #court_custom_name').on('change', function() {
    courts_indexes_select_update();
  });
...
function redraw_court_generated_name() {
  ...
  var custom_name = $('#court_custom_name').val();
  if (custom_name) type = custom_name;
