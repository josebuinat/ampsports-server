class ReservationsCartFormItem extends React.Component {
  ...
  _on_date_change(e) {
    ...
    const start_time = this._r().start.format('HH:mm')
    const end_time = this._r().end.format('HH:mm')
  ...
  _on_start_time_change(e) {
    const date = this._r().start.format('DD/MM/YYYY')
  ...
  _on_end_time_change(e) {
    const date = this._r().end.format('DD/MM/YYYY')
    const time = e.target.value
    reservations_cart_store.update_reservation(this._r().key, {
