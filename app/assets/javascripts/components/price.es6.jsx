...
class Price extends React.Component {
    ...
    return (
      <Modal.Header>
        ...
        <p>
          ...
          {I18n.t('venues.manage_price.price')} <strong>{this.state.price.value}{this.props.currencyUnit}</strong>
        </p>
      </Modal.Header>
    );
  }
  ...
  content() {
    const courts = this.state.price.courts.map(function(court, index) {
      return(
    ...
    const days = this.state.price.days.map(function(day, index) {
      return(
        <li key={index}>
