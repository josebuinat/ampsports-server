module MembershipImportValidator
  ...
  def find_user
    ...
    unless user.present?
      errors.add(:email, t_error('email', email: import_data[:email]))
    end
  end
  ...
  def validate_already_imported
    ...
    similar_membership = Membership.where(venue: venue,
                                          user: user,
                                          price: price,
  ...
  def make_reservations_for_validation
    ...
    time_params = MembershipTimeSanitizer.new(import_data).time_params

    self.make_reservations(time_params, [court.id])
  end
